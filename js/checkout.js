...
async function initialize() {
    const { id, clientSecret } = await fetch("stripe/payment_init.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ request_type: 'create_payment_intent', price: $('#eventPrice').val(), name: $('#eventName').val() }),
    }).then((r) => r.json());
...
async function handleSubmit(e) {
    ...
    let customer_name = document.getElementById("firstname").value + ' ' + document.getElementById("lastname").value;
    let customer_email = document.getElementById("email").value;
    customer_name = customer_name.trim();

    const { id, customer_id } = await fetch("stripe/payment_init.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (paymentIntent) {
        switch (paymentIntent.status) {
            case "succeeded":
                ...
                fetch("stripe/payment_init.php", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ request_type: 'payment_insert', payment_intent: paymentIntent, customer_id: customerID,name:$('#eventName').val(),price:$('#eventPrice').val(),
                    // address_line:$('#address_line').val(),address_line2:$('#address_line2').val()
                 }),
                })
                    .then(response => response.json())
                    .then(data => {
                        if (data.payment_txn_id) {
                            window.location.href = 'stripe/payment-status.php?pid='+data.payment_txn_id;
                        } else {
